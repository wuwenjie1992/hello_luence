...
function say(word) {
	...
	if (cluster.isWorker) console.log("\x1B[33mWorker" 
	+ cluster.worker.id + "\x1B[39m");
}
...
function start(route, handle) {
	...
	function onRequest(request, response) {
		...
		count++;
		console.log("Processing No." + count + " request(s)!\nUptime:" + process.uptime());
...
function howtosendfile(request, response, realPath, file, large) {
	...
	if (! (contentType === undefined)) { //全等（值和类型）
		...
		*/
		...
		if (large == 0) {
			response.writeHead(200, {
				"Content-Disposition": "attachment;" + realPath
			});
			response.write(file, "binary");
			response.end();
		} else {
...
function search(request, response, postData) {
	...
	if (queryStr == '') {
		...
		//执行本地命令
		var shell_name = "java -server -Xms1024m -Xmx1024m -jar " 
		+ jarPath + " -SQPS " + indexPath + " \"" + queryStr + "*\" name " + "\"\";";

		var shell_contents = "java -server -Xms1024m -Xmx1024m -jar " 
		+ jarPath + " -SQPS " + indexPath + " \"" + queryStr + "\" contents " + "\"\";";
...
function execShellCommand(shell, response) {
	...
	function(error, stdout, stderr) {
		...
		if (stderr) {
			...
			//组织结果页面
			if (fsExistSync(resultPage)) {

				fs.readFile(resultPage, "utf8",
				function(err, file) {
					if (err) {
						throw500(response);
					} else {
...
function Shogun(request, response) {
	...
	response.write("your request is " + querystring.parse(query)["q"] + " vs 上杉謙信");
	response.end();
}
...
function(chunk) {
	...
	else process.stdout.write("Your command is " + chunk);
});
